...
mixin (
  ...
  func buildConfirmationEmail(order : OrderLib.OrderPublic, customerName : Text, customerPhone : Text) : Text {
    let addr = order.deliveryAddress;
    ...
    for (item in order.items.values()) {
      ...
      itemRows #= "<tr>"
        # "<td style='padding:8px;border-bottom:1px solid #eee'>" # item.title # "</td>"
        # "<td style='padding:8px;border-bottom:1px solid #eee;text-align:center'>" # item.quantity.toText() # "</td>"
        # "<td style='padding:8px;border-bottom:1px solid #eee;text-align:right'>\u{20B9}" # (item.price / 100).toText() # "." # (item.price % 100).toText() # "</td>"
        # "<td style='padding:8px;border-bottom:1px solid #eee;text-align:right'>\u{20B9}" # (lineTotal / 100).toText() # "." # (lineTotal % 100).toText() # "</td>"
        # "</tr>";
    };
    ...
    "<html><body style='font-family:Arial,sans-serif;color:#333;max-width:700px;margin:0 auto'>"
    # "<div style='background:#8B1A2B;color:white;padding:20px;text-align:center'>"
    # "<h1 style='margin:0'>AssamRoots</h1>"
    # "<p style='margin:4px 0'>Order Confirmation</p>"
    # "</div>"
    # "<div style='padding:24px'>"
    # "<p style='font-size:16px'>Dear " # customerName # ",</p>"
    # "<p>Thank you for your order! Here is your confirmation:</p>"
    # "<h2 style='color:#8B1A2B'>Order #" # order.id.toText() # "</h2>"
    # "<table style='width:100%;margin-bottom:16px'><tbody>"
    # "<tr><td style='color:#666;padding:2px 0;width:150px'>Phone:</td><td style='padding:2px 0'>" # customerPhone # "</td></tr>"
    # "<tr><td style='color:#666;padding:2px 0'>Payment:</td><td style='padding:2px 0'>" # order.paymentMethod # "</td></tr>"
    # "<tr><td style='color:#666;padding:2px 0'>Delivery:</td><td style='padding:2px 0'>" # deliveryTypeText # "</td></tr>"
    # "<tr><td style='color:#666;padding:2px 0'>Est. Delivery:</td><td style='padding:2px 0'>" # estDelivery # "</td></tr>"
    # "</tbody></table>"
    # "<h3 style='color:#8B1A2B;border-bottom:2px solid #8B1A2B;padding-bottom:8px'>Delivery Address</h3>"
    # "<table style='margin-bottom:16px'><tbody>"
    # "<tr><td style='color:#666;padding:2px 0;width:150px'>Name:</td><td style='padding:2px 0'>" # addr.name # "</td></tr>"
    # "<tr><td style='color:#666;padding:2px 0'>Phone:</td><td style='padding:2px 0'>" # addr.phone # "</td></tr>"
    # "<tr><td style='color:#666;padding:2px 0'>House/Flat:</td><td style='padding:2px 0'>" # addr.houseNo # "</td></tr>"
    ...
    # "<tr><td style='color:#666;padding:2px 0'>Pincode:</td><td style='padding:2px 0'>" # addr.pincode # "</td></tr>"
    # "</tbody></table>"
    # "<h3 style='color:#8B1A2B;border-bottom:2px solid #8B1A2B;padding-bottom:8px'>Order Items</h3>"
    # "<table style='width:100%;border-collapse:collapse'>"
    # "<thead><tr style='background:#f8f0f1'>"
    # "<th style='padding:8px;text-align:left'>Product</th>"
    # "<th style='padding:8px;text-align:center'>Qty</th>"
    # "<th style='padding:8px;text-align:right'>Unit Price</th>"
    # "<th style='padding:8px;text-align:right'>Total</th>"
    ...
    # "</tbody></table>"
    # "<table style='width:100%;margin-top:16px'><tbody>"
    # "<tr><td style='text-align:right;padding:4px 0'>Subtotal:</td><td style='text-align:right;padding:4px 0;width:120px'>\u{20B9}" # (subtotal / 100).toText() # "." # (subtotal % 100).toText() # "</td></tr>"
    # "<tr><td style='text-align:right;padding:4px 0'>" # deliveryTypeText # " Cost:</td><td style='text-align:right;padding:4px 0'>\u{20B9}" # (deliveryCost / 100).toText() # "." # (deliveryCost % 100).toText() # "</td></tr>"
    # "<tr style='font-weight:bold;font-size:16px'><td style='text-align:right;padding:8px 0;border-top:2px solid #8B1A2B'>Grand Total:</td><td style='text-align:right;padding:8px 0;border-top:2px solid #8B1A2B'>\u{20B9}" # (grandTotal / 100).toText() # "." # (grandTotal % 100).toText() # "</td></tr>"
    # "</tbody></table>"
    # "</div>"
    # "<div style='background:#f8f0f1;padding:16px;text-align:center;color:#666;font-size:13px'>"
    # "<p>Questions? Email us at <a href='mailto:assamshop@assamroots.shop'>assamshop@assamroots.shop</a></p>"
    # "<p>AssamRoots \u{2014} Connecting you with authentic Assamese products</p>"
    # "</div>"
    # "</body></html>";
  };
  ...
  public shared ({ caller }) func createOrder(input : OrderLib.CreateOrderInput) : async OrderLib.OrderPublic {
    ...
    let customerEmail = switch (profile) { case (?p) p.email; case null "" };
    let adminHtml = buildReceiptEmail(orderPublic, customerName, customerPhone);
    ...
    // Send confirmation email to customer — only if email is available
    if (customerEmail != "") {
      let confirmHtml = buildConfirmationEmail(orderPublic, customerName, customerPhone);
      ignore EmailClient.sendServiceEmail(
        "orders",
        [customerEmail],
        "Order Confirmed \u{2014} AssamRoots #" # orderPublic.id.toText(),
        confirmHtml,
      );
    };
